public class IntegerToEnglishWords {
    ...
    // Append a number within a thousand
    void appendNumber(int number, int thousandBase, StringBuilder sb) {
        if (number == 0) return;
        int hundreds = number / 100;
        if (hundreds > 0) sb.append(oneNames[hundreds - 1]).append(" Hundred").append(' ');
        ...
        if (thousandBase > 0) sb.append(thousandNames[thousandBase - 1]).append(' ');
    }
